...
6. Workers consume event: AV scan, FTS index, embedding. _(Thumbnails are NOT worker-generated — Cloudinary derives them on delivery via `getThumbnailUrl`.)_
...
- Workers talk to: Postgres, Redis, storage (read for AV), LLM, OpenFGA. _(Thumbnails are derived on delivery by Cloudinary, not read/generated by a worker.)_
